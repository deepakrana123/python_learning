...
class TreeAncestor:
    def __init__(self, n, parent):
        ...
        self.dp=[[-1]*(self.LOG+1) for _ in range(n)]
        self.depth = [-1] * n
    ...
    def dfs(self,currNode,parent):
        ...
        if parent!=-1:
            self.depth[currNode]=self.depth[parent]+1
        for i in range(1,self.LOG+1):
            if self.dp[currNode][i-1]!=-1:
                self.dp[currNode][i]=self.dp[self.dp[currNode][i-1]][i-1]
        for child in self.tree[currNode]:
            if child!=parent:
                self.depth[child]=self.depth[currNode]+1
                self.dfs(child,currNode)
...
b=TreeAncestor(7,[-1, 0, 0, 1, 1, 2, 2])
print(b.getKthAncestor(6,3))
